...
def main() :
    ...
    @st.cache
    def load_data():
        z = ZipFile("data/raw_data.zip")
        data = pd.read_csv(z.open('raw_data.csv'), index_col='SK_ID_CURR', encoding ='utf-8')
...
# Charte et CSS
    ...
    st.sidebar.markdown("<a href='#S6'>&nbsp;&nbsp;&nbsp;<font color=blue size=+2><b>Aide:</b><I> Exemple de lecture des résultat</font></a>", unsafe_allow_html=True)
    st.sidebar.markdown(" Ex  - client non solvable : id 194347", unsafe_allow_html=True) 
    st.sidebar.markdown("&nbsp; &nbsp; &nbsp;- client  solvable: 374600", unsafe_allow_html=True) 
    ...
    if prediction <= seuil_solvabilite :
        decision = "<font size=+5 color='green'><b>PRÊT ACCORDÉ</b></font>" 
    else:
        decision = "<font size=+5 color='red'><b>PRÊT REJETÉ</b></font>"
        


    #st.write("prediction ".format(float(prediction))

    #st.markdown(" Score :  {:.0f} ".format(round(float(prediction)*100, 1)), unsafe_allow_html=True))


    html_str = f""" <font size=+5> Decision avec seuil {seuil_solvabilite}  &nbsp;</font> : {decision}"""

    st.markdown(html_str, unsafe_allow_html=True)
    #st.write("  {seuil_solvabilite} ):", decision, unsafe_allow_html=True)
    
 

    html_str = f""" <font size=+5> Score :  {round(float(prediction)*100, 1)} </font>"""

    st.markdown(html_str, unsafe_allow_html=True)
    
    
    html_str = f"""
    <font color=blue><i> Le score reflète la pertinence de la décision, il est compris entre 0 et 100: <br>
     <b> &nbsp;&nbsp;- scrore <=10 :</b> Le client est solvable <br>
     <b> &nbsp;&nbsp;- scrore >10 :</b> Le client est non solvable</font>
    """
    st.markdown(html_str, unsafe_allow_html=True) 
    ...
    X = sample.iloc[:, :-1]
    X = X[X.index == chk_id]
    number = st.slider("Selectionnez le nombre de features…", 0, 20, 5)
    ...
    st.markdown("<font color=maroon size=+3><b><u> Feature description</u></b></font>", unsafe_allow_html=True) 
    list_features = description.index.to_list()
    feature = st.selectbox('sélectionnez la feature dans la liste...', list_features)
    ...
    st.markdown("<a name='S6'>", unsafe_allow_html=True) 
    st.markdown("<a href='#S6'><font color=maroon size=+5><b> Aide:</b><I> Exemple de lecture des résultat</font></a>", unsafe_allow_html=True)
        
 
    
    from PIL import Image 
    #opening the image
    image = Image.open('help_interpretabilite.PNG')



    #displaying the image on streamlit app

    st.image(image, caption='Aide sur linterprétabilité des modèles')
